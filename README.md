...
- 若频繁发送 update（>= 1/秒）：不需要单独心跳
- 若更新不频繁（< 1/秒）：每 30 秒发送一次心跳保活

---

### 4. Timeout Notification（超时通知）
